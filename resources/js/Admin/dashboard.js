...
function loadChart() {
    ...
    fetch(
        ...
        .then(res => {
            ...
            document.getElementById("teksPerbandingan").textContent = res.summary.compare_text;
...
function loadAverageConsultationTime() {
    fetch(`/dashboard/average-waktukonsultasi`)
        ...
        .then(res => {
            ...
            document.getElementById("avgTimeDisplay").textContent = res.average_time;
            document.getElementById("avgPercentage").textContent = res.percentage + "%";
            document.getElementById("avgCompareText").textContent = res.compare_text;

            // Perbarui warna dan ikon panah berdasarkan persentase
            const avgPersenWrapper = document.getElementById("avgPersenWrapper");
...
function loadNewPatientsSummary() {
    fetch(`/dashboard/getnewpatients`)
        ...
        .then(res => {
            ...
            document.getElementById("newPatientsPercentage").textContent = res.percentage + "%";
            document.getElementById("newPatientsCompareText").textContent = res.compare_text;
...
function loadRegisteredPatientsSummary() {
    fetch(`/dashboard/getregisteredpatients`)
        .then(res => res.json())
        .then(res => {
            document.getElementById("registeredPatientsTotal").textContent = res.total;
            ...
            if (res.percentage >= 0) {
                wrapper.classList.add("bg-green-300");
                arrow.setAttribute("d", "M640-720v80h104L536-434 376-594 80-296l56 56 240-240 160 160 264-264v104h80v-240H640Z");//top arrow
                arrow.setAttribute("fill", "#008000"); 
            } else {
                wrapper.classList.add("bg-red-300");
                arrow.setAttribute("d", "M640-240v-80h104L536-526 376-366 80-664l56-56 240 240 160-160 264 264v-104h80v240H640Z"); //down arrow
